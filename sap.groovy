def abap_unit(LABEL,HOST,CREDENTIAL,PACKAGE,COVERAGE) {	
	...
	withCredentials([usernamePassword(credentialsId: CREDENTIAL, usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
		stage('[' + LABEL + '] ABAP Unit') {
			dir('sap-pipeline') {
				sh "newman run sap-pipeline/abap_unit_coverage.postman_collection.json --insecure --bail " +
...
def abap_sci(LABEL,HOST,CREDENTIAL,PACKAGE,VARIANT) {	
	...
	withCredentials([usernamePassword(credentialsId: CREDENTIAL, usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {	
		stage('[' + LABEL + '] ABAP Code Inspector') {
			dir('sap-pipeline') {
					sh "newman run sap-pipeline/abap_sci.postman_collection.json --insecure --bail " +
					"--environment sap-pipeline/EH7.postman_environment.json " +
					"--timeout-request 120000 " +
					"--global-var host=$HOST " +
					"--global-var username=$USERNAME " +
...
def sap_api_test(LABEL,HOST,CREDENTIAL) {
	...
	withCredentials([usernamePassword(credentialsId: CREDENTIAL, usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
		stage('[' + LABEL + '] SAP API Tests') {
			dir('sap-pipeline') {
				try {
					sh "newman run sap-pipeline/SimpleRESTTest.postman_collection.json --insecure --bail " + 
					"--environment sap-pipeline/EH7.postman_environment.json " + 
					"--reporters junit " +
